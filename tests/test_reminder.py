...
运行：python -m pytest tests/ -v  或  python tests/test_reminder.py
"""
import asyncio
import sys
import time
...
def make_plugin(users, config=None):
    ...
    p.config = {
        ...
        "remind_message_template": "体力 {remain}/{max} 不足！",
    }
    if config:
...
def test_remind_when_low():
    ctx = FakeContext()
    p = make_plugin({"10001": base_user()})
    p.context = ctx
    p.client = FakeClient(EnergyInfo(remain=30, max=240, total_second=3600))

    asyncio.run(p._check_reminders())

    assert len(ctx.sent) == 1, "体力低时应发送提醒"
    ...
    assert "30/240" in chain.chain[1].text
    # 发送后记录时间戳
    assert p._users["10001"]["last_remind_ts"] > 0


def test_no_remind_when_high():
    ctx = FakeContext()
    p = make_plugin({"10001": base_user()})
    p.context = ctx
    p.client = FakeClient(EnergyInfo(remain=200, max=240, total_second=0))

    asyncio.run(p._check_reminders())

    assert ctx.sent == [], "体力高时不应提醒"
...
def test_cooldown():
    ...
    p.context = ctx
    p.client = FakeClient(EnergyInfo(remain=30, max=240))
...
def test_threshold_disabled():
    ctx = FakeContext()
    p = make_plugin({"10001": base_user(threshold=0)})  # 关闭
    p.context = ctx
    p.client = FakeClient(EnergyInfo(remain=30, max=240))
...
def test_token_expired_skips_without_crash():
    ...
    p.context = ctx
    p.client = FakeClient(EnergyInfo(remain=30, max=240), raise_auth=True)
...
if __name__ == "__main__":
    test_remind_when_low()
    test_no_remind_when_high()
    test_cooldown()
