...
体力**达到或超过设定阈值**时，后台定时任务会在绑定所在的群 **@ 你** 并发送提醒文案（提醒你快去清体力）。
默认阈值为 230（即体力快满了就提醒），可用 `/体力提醒 <数值>` 修改。
同一用户两次提醒之间至少间隔 `remind_cooldown_minutes`（默认 120 分钟），避免刷屏。

**自动刷新**：插件内置两个后台定时任务——
- **整点自动刷新**：每个整点自动刷新所有绑定用户的体力数据，保持数据新鲜
- **提醒检查**：每隔 `check_interval_minutes` 分钟检查一次，体力达到阈值则 @ 提醒
...
3. 在插件配置中按需调整：
   ...
   - `default_threshold`：默认提醒阈值（默认 230，体力达到此值时提醒）
